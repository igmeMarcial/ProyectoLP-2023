				...
				<div class="row">
					<div class="col-xl">
						<div class="card mb-4">
							...
							<div class="card-body">
								<form action="ControladorInventario?accion=actualizarInventario" method="post">
								<input name="idInventario" type="hidden"value="${getInventario.idInventario}" />
									<div class="mb-3">
										<label class="form-label" for="prov">Proveedor:</label>
											<select id="prov" name="proveedor"  aria-label="Default select example" required>
												<c:forEach items="${listadoProv}" var="prov">											  		
												  <option value="${prov.idProveedor}" <c:if test="${prov.idProveedor == getInventario.idProveedor}" var="condition"> selected</c:if> > ${prov.razonSocial}</option>
												</c:forEach>										  
											</select>								    
									</div>
									<div class="mb-3">
										<label class="form-label" for="equipo">Equipo:</label>
											<select id="equipo" name="equipo"  aria-label="Default select example" required>
												<c:forEach items="${listadoEquipos}" var="equipo">											  		
												  <option value="${equipo.idEquipo}" <c:if test="${equipo.idEquipo == getInventario.idEquipo}" var="condition"> selected</c:if> > ${equipo.nombre}</option>
												</c:forEach>										  
											</select>								    
									</div>
									<div class="mb-3">
										<label class="form-label" for="categoria">Categoria:</label>
											<select id="categoria" name="categoria"  aria-label="Default select example" required>
												<c:forEach items="${listadoCategorias}" var="cat">											  		
												  <option value="${cat.idCategoria}" <c:if test="${cat.idCategoria == getInventario.idCategoria}" var="condition"> selected</c:if> > ${cat.nombreCategoria}</option>
												</c:forEach>										  
											</select>								    
									</div>									
										...
										<label class="form-label" for="ticket">Ticket:</label>
											<select id="ticket" name="ticket"  aria-label="Default select example" required>
												<c:forEach items="${listadoTicket}" var="ti">											  		
												  <option value="${ti.idTicket}" <c:if test="${ti.idTicket == getInventario.idTicket}" var="condition"> selected</c:if> > ${ti.nombre}</option>
												</c:forEach>										  
											</select>								    
									</div>
									<div class="mb-3">
										...
										<div class="input-group input-group-merge">
											...
											<input name="stockInicial" type="number" class="form-control inputCustom borderColorInput" id="stockInicial" placeholder="Stock Inicial" aria-label="apellido" aria-describedby="basic-icon-default-fullname2" 
											value="${getInventario.stockInicial}" required/>
										</div>
									</div>
									<div class="mb-3">
										...
										<div class="input-group input-group-merge">
											<span id="basic-icon-default-fullname2" class="input-group-text labelCustom borderColorInput"><i class="ri-folder-add-line"></i></span>
											<input name="salidas" type="number" class="form-control inputCustom borderColorInput" id="salidas" placeholder="Salidas" aria-label="apellido" aria-describedby="basic-icon-default-fullname2" 
											 value="${getInventario.salidas}" required/>
										</div>
									</div>
									<div class="mb-3">
										...
										<div class="input-group input-group-merge">
											<span id="basic-icon-default-fullname2" class="input-group-text labelCustom borderColorInput"><i class="ri-folder-add-line"></i></span>
											<input name="total" type="number" class="form-control inputCustom borderColorInput" id="total" placeholder="Total" aria-label="apellido" aria-describedby="basic-icon-default-fullname2" 
											 value="${getInventario.total}" required/>
										</div>
									</div>
									
								
									<button id="submitUpdate" type="submit" class="btn btn-primary">Actualizar</button>
								</form>
							</div>
						</div>
					</div>
